...
class VAE(Module):
    ...
    def chain(self, x, n_z=None):
        ...
        with tf.name_scope('VAE.chain'):
            ...
            if n_z is not None:
                # z_group_ndims = validate_group_ndims(self.z_group_ndims,
                #                                      'z_group_ndims')
                # z_rank = tf.rank(q_net['z'])
                # assert_op = tf.assert_less(
                #     z_group_ndims, z_rank,
                #     message='`z_group_ndims` should be at least one less '
                #             'than the rank of sampled `z`, when `n_z` is '
                #             'not None')
                # with tf.control_dependencies([assert_op]):
                #     # We should exclude the dimensions of `z_group_ndims`,
                #     # since this latent-axis is used by variational solvers,
                #     # while `z_group_ndims` should have been reduced before
                #     # variational solvers have ever been called.
                #     latent_axis = -(z_rank - z_group_ndims)
                # TODO: activate above code once ZhuSuan supports dynamic axis
                latent_axis = 0
            else:
                latent_axis = None
    ...
    def _forward(self, inputs, n_z=None, **kwargs):
        ...
        """
        q_net = self.variational(inputs, z=None, n_z=n_z)
        return q_net['z']
