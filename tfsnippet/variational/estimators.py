...
from tfsnippet.nn import log_sum_exp, tfops, log_mean_exp
from .utils import _require_multi_samples

__all__ = ['sgvb_estimator', 'iwae_estimator']
...
def sgvb_estimator(values, axis=None, keepdims=False, name=None):
    ...
    Args:
        values: Values of :math:`f(\\mathbf{z},\\mathbf{x})`,
    ...
    Returns:
        tf.Tensor: The gradient estimator.
    """
    estimator = tf.convert_to_tensor(values)
    with tf.name_scope(name, default_name='sgvb_estimator', values=[estimator]):
        if axis is not None:
...
def iwae_estimator(log_values, axis, keepdims=False, name=None):
    ...
    Args:
        log_values: Values of :math:`\\log f(\\mathbf{z},\\mathbf{x})`,
            computed with :math:`\\mathbf{z} \\sim q(\\mathbf{z}|\\mathbf{x})`.
        axis: The sampling dimensions to be averaged out.
        keepdims (bool): When `axis` is specified, whether or not to keep
            the averaged dimensions?  (default :obj:`False`)
