...
def nvil_estimator(values, latent_log_joint, baseline=None,
    ...
    Args:
        ...
        l_signal: If specified, will use `l_signal` instead of `values`
            when training :math:`C_{\\psi}(\\mathbf{x})` and `c`, as well
            as centering :math:`\\log q_{\\phi}(\\mathbf{z}|\\mathbf{x})`.

            This argument is originally designed for a slight optimization
            in :meth:`tfsnippet.VariationalInference.nvil`.
    ...
    if l_signal is not None:
        l_signal = tf.convert_to_tensor(l_signal)
